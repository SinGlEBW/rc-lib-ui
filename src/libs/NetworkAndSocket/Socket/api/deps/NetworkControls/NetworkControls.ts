import { NetworkStatusTracker } from "dev-classes";
...
type OnStatusChange = (info: { isNetwork: boolean; typeNetwork: string }) => void;
...
export class NetworkControls {
  ...
  async motitorNetwirk(cbInterval: NetworkMonitorCallbackInterval, cb: NetworkMonitorCallback, options: NetworkControlsConfigProps = {}) {
    ...
    if (!isActiveDelay) {
      ...
      if (currentNetworkInfo?.isNetwork) {
        ...
        const { stop } = networkTicker?.fetchingNetwork(
          (info) => {
            cbInterval({
              reset: () => {
                stop();
